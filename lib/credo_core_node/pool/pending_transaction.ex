defmodule CredoCoreNode.Pool.PendingTransaction do
  use Mnesia.Schema, table_name: :pending_transactions, fields: [:hash, :nonce, :to, :value, :fee, :data, :v, :r, :s]
  ...
  def unsigned_rlp(%PendingTransaction{} = tx) do
    ExRLP.encode(tx.nonce, encoding: :binary) <>
      ...
      ExRLP.encode(tx.fee, encoding: :binary) <>
      ExRLP.encode(tx.data, encoding: :binary)
  end
  ...
  def signed_rlp(unsigned_rlp, v, r, s) do
    unsigned_rlp <>
      ...
      ExRLP.encode(r, encoding: :binary) <>
      ExRLP.encode(s, encoding: :binary)
  end
