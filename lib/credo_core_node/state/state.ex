defmodule CredoCoreNode.State do
  ...
  def calculate_world_state!(txs) when is_list(txs) do
    ...
    start_root
    |> calculate_world_state!(start_block_number, end_block_number)
    |> calculate_world_state!(txs)
  end

  defp calculate_world_state!(_start_root, _start_block_number, -1), do: nil

  defp calculate_world_state!(start_root, start_block_number, end_block_number)
       when start_block_number >= end_block_number,
       do: start_root
  ...
  defp last_existing_state(-1), do: {nil, -1}
      ...
      %{hash: hash, state_root: root} ->
        ...
        cond do
          ...
          Trie.Node.decode_trie(trie) == :empty ->
            Exleveldb.close(elem(trie.db, 1))
            if block_number > 0, do: last_existing_state(block_number - 1), else: {nil, -1}
